"""Product-local MD-resolution walker for CDS — discount curve only.

Per the orchestrator's MD walker
for CDS pricing is **discount-curve only**: credit curves carry their
own hazard/probability inputs (the assembler validates this in
:func:`_validate_credit_curve_payload`) and never reach this module.

The walker collects every ``quote_id`` field on the resolved discount
curve and substitutes the resolved value via the snapshot pin (when
present) or the MD client (otherwise). The output is the
``list[ResolvedQuoteValue]`` echoed back in the response envelope and
that the engine-side encoder uses to build the FlatBuffers payload —
quote IDs never leave the orchestrator (north-star invariant #8).

This module is **product-local on purpose** — each product ships a
slimmed-down walker tuned to its own curve shape; collapsing the
common surface into a shared walker is a candidate future refactor.
...
* Per-item ``found=False`` markers from ``MdClient.resolve_quotes``
  ...
  ``cds_quote_resolution_failed`` (422) with every missing canonical
  ID listed in ``details``.
"""
...
# Same shape as swap_ir / bonds walkers; the portal stores helpers
# with a top-level ``quote_id`` field and discriminated-helper
# wrappers ({"point_type": "...", "point": {...}}) nest the field
# under ``point.quote_id``, so the walker checks both.
...
# ``details`` so the operator has the complete picture.
...
def collect_quote_ids(curves: Iterable[ResolvedCurve]) -> list[str]:
    ...
    body is stable (helps debugging via diff'ing two runs). The
    CDS endpoint only walks the discount curve; we keep the
    parameter typed as an iterable so the future portfolio shape
    doesn't need a signature churn.
    """
...
async def resolve(
    *,
    ...
    collected into a single 422 (``cds_quote_resolution_failed``).
    """
    ...
    if misses:
        try:
            ...
            _log.warning(
                "orchestrator.pricing.cds.md_resolution_error",
