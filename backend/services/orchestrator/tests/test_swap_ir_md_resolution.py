"""Unit tests for ``pricing/swap_ir/md_resolution.py``.
...
* Surface per-MD-error envelopes verbatim
  ...
  ``swap_ir_quote_resolution_failed`` (422).
...
class _FakeMdClient:
    ...
    the test stays focused. Mypy-wise the walker takes ``MdClient``;
    we cast at the call site to keep the suite from importing the
    full client just for a one-method stub.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # captured alongside ``calls`` (not folded into
        # the tuple) so legacy positional assertions stay unchanged.
        self.snapshot_versions: list[str | None] = []
    ...
    async def resolve_quotes(
        ...
        *,
        ...
        self.snapshot_versions.append(snapshot_version)
...
async def test_resolve_pins_from_snapshot_short_circuit_md() -> None:
    ...
    assert out[1].source == "EOD-USD"  # fallback to snapshot name when source is None
    assert md.calls == []  # everything pinned → no upstream call
...
async def test_resolve_mix_of_snapshot_pins_and_live_md() -> None:
    ...
    curves = [
        _curve(
            points=[
                {"quote_id": "USD.IRS.1Y"},  # pinned
                {"quote_id": "USD.IRS.10Y"},  # live
            ]
        )
    ]
...
async def test_resolve_per_item_not_found_raises_swap_ir_quote_resolution_failed() -> None:
    md = _FakeMdClient(
        results=[
            _resolved_quote(canonical_id="USD.IRS.1Y", value=4.25),
    ...
    err = excinfo.value
    assert err.code == "swap_ir_quote_resolution_failed"
...
async def test_resolve_truncates_breadcrumb_id_list_in_quote_not_found_detail() -> None:
    ...
    When the upstream returns 404, ``map_md_client_error`` builds a
    :class:`QuoteNotFoundError` whose detail string echoes the
    breadcrumb canonical_id list. With >5 misses the walker
    truncates so a 1k-quote batch doesn't render a multi-KB detail.
    The full miss list still lands in the (separately surfaced)
    422's ``details`` payload.
    """
    ...
    # The breadcrumb has 5 IDs + ``...`` in the detail message.
    detail_text = str(excinfo.value.detail)
    assert "..." in detail_text
    # Trailing IDs (6Y..10Y) must NOT appear in the breadcrumb.
    assert "USD.IRS.10Y" not in detail_text


# ---------------------------------------------------------------------------
# snapshot_version wire-through
# ---------------------------------------------------------------------------


async def test_resolve_forwards_snapshot_version_to_md_client() -> None:
    ...
    await resolve(
        ...
        snapshot=None,
        md_client=md,  # type: ignore[arg-type]
        snapshot_version="etag-A",
    )

    assert md.snapshot_versions == ["etag-A"]


async def test_resolve_backward_compat_no_snapshot_version() -> None:
    # Today's default: walker called without snapshot_version -> MD
    # client receives ``None`` -> body omits the field, cache keys
    # collapse to the (cid, as_of, "-") shape per quote_cache_key.
    md = _FakeMdClient(results=[_resolved_quote(canonical_id="USD.IRS.1Y", value=4.25)])
    curves = [_curve(points=[{"quote_id": "USD.IRS.1Y"}])]
    ...
    assert md.snapshot_versions == [None]


async def test_resolve_stale_snapshot_pin_surfaces_quote_resolution_failed() -> None:
    # Negative: when the MD service can't find the canonical_id under
    # the pinned etag (stale pin / trigger advanced), every result is
    # found=False and the walker collapses into a single
    # swap_ir_quote_resolution_failed (422). This is the "no new code
    # invented" path the operator brief calls for.
    md = _FakeMdClient(
        results=[
            _resolved_quote(canonical_id="USD.IRS.1Y", value=0.0, found=False),
            _resolved_quote(canonical_id="USD.IRS.5Y", value=0.0, found=False),
        ]
    )
    curves = [_curve(points=[{"quote_id": "USD.IRS.1Y"}, {"quote_id": "USD.IRS.5Y"}])]
    ...
    with pytest.raises(SwapIrQuoteResolutionFailedError) as excinfo:
        await resolve(
            ...
            snapshot=None,
            md_client=md,  # type: ignore[arg-type]
            snapshot_version="etag-stale",
        )
    assert excinfo.value.code == "swap_ir_quote_resolution_failed"
    assert md.snapshot_versions == ["etag-stale"]
